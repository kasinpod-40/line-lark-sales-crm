...
type PaymentUiState = { message_id?: string; draft_id?: string };
...
export function isPaymentCardAction(action: string): boolean {
...
export class PaymentActionService {
  ...
  private uiStateId(caseId: string): string {
    return `ui:payment-card:${caseId}`;
  }

  private async readUiState(caseId: string): Promise<PaymentUiState> {
    const row = await this.env.DB.prepare(
      "SELECT payload_json FROM interaction_drafts WHERE draft_id=? LIMIT 1",
    ).bind(this.uiStateId(caseId)).first<UnknownRecord>();
    if (!row) return {};
    try {
      const parsed: unknown = JSON.parse(asString(row.payload_json, "{}"));
      return isRecord(parsed) ? {
        message_id: asString(parsed.message_id) || undefined,
        draft_id: asString(parsed.draft_id) || undefined,
      } : {};
    } catch {
      return {};
    }
  ...
  private async rememberUiState(caseId: string, createdBy: string, state: PaymentUiState): Promise<void> {
    const now = Date.now();
  ...
  private async updateDraftPayload(draftId: string, payload: PaymentDraft): Promise<void> {
    const result = await this.env.DB.prepare(
      "UPDATE interaction_drafts SET payload_json=? WHERE draft_id=? AND kind='payment' AND status='PENDING'",
    ).bind(JSON.stringify(payload), draftId).run();
  ...
  private async completedDraft(draftId: string): Promise<{ caseId: string; createdBy: string; payload: PaymentDraft } | null> {
    const row = await this.env.DB.prepare(
      "SELECT case_id,created_by,payload_json,status,kind FROM interaction_drafts WHERE draft_id=? LIMIT 1",
    ...
    try {
      ...
      return {
        ...
        payload: payload as unknown as PaymentDraft,
      };
    } catch {
      return null;
    }
  ...
  private async patchOrCreatePreview(route: CaseRoute, operatorOpenId: string, draftId: string, payload: PaymentDraft, preferredMessageId?: string): Promise<void> {
    const state = await this.readUiState(route.case_id);
    const messageId = preferredMessageId || state.message_id;
    const card = buildPaymentSinglePreviewCard(route.case_id, draftId, payload.amount, payload.note);
    if (messageId) {
      try {
        await this.lark.patchCard(messageId, card);
        await this.rememberUiState(route.case_id, operatorOpenId, { message_id: messageId, draft_id: draftId });
        return;
      } catch {
        // Message may have been deleted; create one replacement below.
      }
    }
    const createdMessageId = await this.lark.replyCard(this.requireRoot(route), card);
    if (!createdMessageId) throw new Error("Lark ไม่คืน message_id ของ QR Card");
    await this.rememberUiState(route.case_id, operatorOpenId, { message_id: createdMessageId, draft_id: draftId });
  }

  private async openPreview(route: CaseRoute, operatorOpenId: string, amount: number, note?: string, preferredMessageId?: string): Promise<void> {
    ...
    const oldState = await this.readUiState(route.case_id);
    if (oldState.draft_id) await this.operational.finishDraft(oldState.draft_id, "CANCELLED").catch(() => undefined);
    ...
    await this.patchOrCreatePreview(route, operatorOpenId, draftId, payload, preferredMessageId);
  }
  ...
  async handle(event: CardActionEvent): Promise<void> {
    ...
    const acquired = await this.operational.acquireAction(actionKey, event.action || "payment_action");
    if (!acquired) return;
    ...
    try {
      ...
      switch (event.action) {
        case "open_qr_form": {
          ...
          await this.openPreview(route, event.operatorOpenId, amount);
          break;
        }
        ...
        case "submit_qr_preview": {
          ...
          await this.openPreview(route, event.operatorOpenId, amount, note, event.messageId);
          break;
        }
        ...
        case "edit_qr_amount": {
          ...
          const messageId = event.messageId || (await this.readUiState(route.case_id)).message_id;
          if (!messageId) throw new Error("ไม่พบ QR Card สำหรับแก้ไขยอด");
          await this.lark.patchCard(messageId, buildPaymentEditCard(route.case_id, draftId, draft.payload.amount, draft.payload.note));
          break;
        }
        ...
        case "cancel_qr_edit": {
          ...
          if (!draft || draft.kind !== "payment" || draft.case_id !== route.case_id || draft.created_by !== event.operatorOpenId) throw new Error("QR draft หมดอายุหรือไม่ถูกต้อง");
          const messageId = event.messageId || (await this.readUiState(route.case_id)).message_id;
          if (!messageId) throw new Error("ไม่พบ QR Card");
          await this.lark.patchCard(messageId, buildPaymentSinglePreviewCard(route.case_id, draftId, draft.payload.amount, draft.payload.note));
          break;
        }
        ...
        case "save_qr_amount": {
          ...
          const messageId = event.messageId || (await this.readUiState(route.case_id)).message_id;
          if (!messageId) throw new Error("ไม่พบ QR Card");
          await this.lark.patchCard(messageId, buildPaymentSinglePreviewCard(route.case_id, draftId, payload.amount, payload.note));
          break;
        }
        ...
        case "cancel_qr_single_card": {
          ...
          const draft = await this.operational.getDraft<PaymentDraft>(draftId);
          if (draft) {
            if (draft.case_id !== route.case_id || draft.created_by !== event.operatorOpenId) throw new Error("QR draft หมดอายุหรือไม่ถูกต้อง");
            await this.operational.finishDraft(draftId, "CANCELLED");
          }
          const messageId = event.messageId || (await this.readUiState(route.case_id)).message_id;
          if (messageId) {
            await this.lark.patchCard(messageId, buildPaymentCancelledCard());
            await this.rememberUiState(route.case_id, event.operatorOpenId, { message_id: messageId });
          }
          break;
        }
        ...
        case "confirm_qr_single_card": {
          ...
          if (!draft) {
            ...
            if (completed && completed.caseId === route.case_id && completed.createdBy === event.operatorOpenId) {
              const messageId = event.messageId || (await this.readUiState(route.case_id)).message_id;
              if (messageId) {
                await this.lark.patchCard(messageId, buildPaymentSentCard(completed.payload.amount));
                await this.rememberUiState(route.case_id, event.operatorOpenId, { message_id: messageId, draft_id: draftId });
              }
          ...
          if (draft.kind !== "payment" || draft.case_id !== route.case_id || draft.created_by !== event.operatorOpenId) throw new Error("QR draft หมดอายุหรือไม่ถูกต้อง");

          const promptPayPayload = buildPromptPayPayload(this.env.PROMPTPAY_TARGET, draft.payload.amount, this.env.PROMPTPAY_TARGET_TYPE ?? "phone");
          ...
          // Never mark the Deal as QR Sent unless the same public PNG URL that
          // LINE will render is reachable and actually returns PNG bytes.
          ...
          const messageId = event.messageId || (await this.readUiState(route.case_id)).message_id;
          if (messageId) {
            await this.lark.patchCard(messageId, buildPaymentSentCard(draft.payload.amount)).catch((error) => {
          ...
          await this.lark.replyText(this.requireRoot(route), `✅ ส่ง PromptPay QR ฿${draft.payload.amount.toLocaleString("th-TH")} เข้า LINE แล้ว`);
          break;
        }
      ...
      if (route?.root_message_id) {
        await this.lark.replyText(route.root_message_id, `❌ ${error instanceof Error ? error.message : String(error)}`).catch(() => undefined);
      }
      throw error;
    }
  }
