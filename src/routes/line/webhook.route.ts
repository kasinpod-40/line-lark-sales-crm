...
function parseSupportedMessageEvent(destination: string, event: unknown): LineEventQueueMessage | null {
  ...
  if (!isRecord(source) || !isRecord(message)) return null;
  ...
  const messageType = getString(message.type) as LineQueueMessageType;
  ...
  return {
    schema_version: 1,
    channel: "LINE",
    webhook_event_id: getString(event.webhookEventId).trim() || `line-webhook-${messageId}`,
    destination,
    is_redelivery: deliveryContext.isRedelivery === true,
    occurred_at: getNumber(event.timestamp, Date.now()),
    source_type: sourceType,
    user_id: userId,
    group_id: getString(source.groupId).trim() || undefined,
    room_id: getString(source.roomId).trim() || undefined,
    message: {
      id: messageId,
      type: messageType,
      text: getString(message.text).trim() || undefined,
      package_id: getString(message.packageId).trim() || undefined,
      sticker_id: getString(message.stickerId).trim() || undefined,
      content_provider_type: contentProvider.type === "external" ? "external" : "line",
      original_content_url: getString(contentProvider.originalContentUrl).trim() || undefined,
    },
  };
}
...
export async function handleLineWebhook(request: Request, env: Env): Promise<Response> {
  ...
  const queueMessages = events
    .map((event) => parseSupportedMessageEvent(destination, event))
    .filter((event): event is LineEventQueueMessage => event !== null);

  try {
    for (const event of queueMessages) await enqueueLineEvent(env, event);
  } catch {
    return jsonResponse({ ok: false, message: "Queue unavailable" }, 503);
  }

  return jsonResponse({ ok: true, received_events: events.length, enqueued_events: queueMessages.length });
}
